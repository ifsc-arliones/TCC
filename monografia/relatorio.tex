\documentclass[ruledheader]{abnt} %\documentclass[12pt,a4paper,oneside]{report} -> antigo
...
\chapter{LevantamentoDeRequisitos} % artigo é dividido por sessões
\label{c_levamtamentoDeRequisitos} % serve para referência cruzada
...
%Antes de explicar a implementação, acredito que seja bacana mostrar o cenário que foi utilizado. Tvz mostrar o cenário anterior.
...
\label{s_decisoesDoProjeto} %serve para referência cruzada
...
\label{s_descricaoModulo} %serve para referência cruzada
...
\label{ss_estabelecimentoDechamada} 
  ...
  %NÃO ESQUECER DE ARRUMAS AS REFERÊNCIAS AQUIIIIIIII. RETIRAR ESTE COMENTÁRIO, SOMENTE DEPOIS DE PRONTO.
...
\label{ss_listaContatoEestadoPresenca} %serve para referência cruzada
  ...
  %FIGURA mostrando um exemplo da aplicação enviando a action para o asterisk, e este retornando as requisições. Mostrar ramais conectados no asterisk, a aplicação enviando requisição ao asterisk, este retornando a aplicação, aplicação enviando para o front, e o front mostrando a lista de usuários 
  
  
\subsection{Mensagens Instântaneas}
\label{ss_mensagensIntantaneas} %serve para referência cruzada


\section{Dificuldade encontradas e suas soluções}
\label{s_dificuldadeEsolucoes} %serve para referência cruzada
...
\label{s_ambienteDetestes} %serve para referência cruzada

\section{Avaliação de resultados}
\label{s_avaliacaoResultados} %serve para referência cruzada
...
\label{s_resumoDoTrabalho} %serve para referência cruzada

\section{Dificuldades Encontradas}
\label{s_dificuldadeEncontradas} %serve para referência cruzada

\section{Trabalhos Futuros}
\label{s_trabalhosFuturos} %serve para referência cruzada
